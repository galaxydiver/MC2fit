...
class Masking():
    ...
    def __do_suppress_center(self, obj, mask, center_size, is_center_circle=False, is_touch=False, suppress_center=True):
        ...
        if(is_touch): ## Masking objects when the boundaries of the masking regions are inside the central region
            if(is_center_circle):
                center_group=masking_sum(mask_output, make_aperture_mask(200, center_size)) ## Count from 1 (0 is background)
            else:
...
def masking_sum(prev_mask=None, new_mask=None):
    ...
    if(hasattr(prev_mask, "__len__")):
        if(hasattr(new_mask, "__len__")):
            previous_mask_max=np.nanmax(prev_mask)+1 ## New mask id = start from max prev_mask
            target=np.where((new_mask!=0))
            new_mask[target]+=previous_mask_max

            return_mask=np.copy(prev_mask)
            target2=np.where((new_mask!=0) & (prev_mask==0))
            return_mask[target2]+=new_mask[target2]
            return return_mask
