...
sum_south_dr10=fits.getdata(fn_legacy_sum_south, ext=1)
...
def download_manager(url, fn_save, overwrite=False, silent=True,
    ...
    """
    if(current_try==0): conf.remote_timeout=timeout
    ...
    try:
        ...
        if(e.code==429): ## Too much requests --> save error separately
            if(current_try<Ntry):
                ...
                download_manager(url, fn_save, overwrite=overwrite, silent=silent, silent_error=silent_error,
                                ...
                                silent_timeerror=True,
                                current_try=current_try+1) ## Restart
        ...
        if(e.code==503): ## Temporarily Unavailable --> save error separately
            np.savetxt(fn_err_tmp, np.array(['http', e]), fmt="%s") ## Ntry cut
...
def download_data(url, fn_save, overwrite=False, silent=False, allow_insecure=True, is_try=True):
    # Download brick data
    if(silent==False):
...
def download_psf(folder, coord,
                 Ntry=5, sleep=2, timeout=10, use_waittime_rand=True,
                 fnkey_psf_south_dr9='psf_dr9_s.fits',
                 fnkey_psf_north_dr9='psf_dr9_n.fits',
                 fnkey_psf_south_dr10='psf_dr10_s.fits',
...
class LegacyCrawling():
    ...
    def __post_init__(self):
        ...
        self.cutout_url+='&pixscale=%0.3f'%plate_scale
    ...
    def psf(self, mode='default', manual_seeing_psf=None, overwrite=None, is_ignore_err=None, return_2dmap=False):
        ...
        self.fn_psf=self.dir_work+'psf_'+self.dr+"_"+mode+"_"+self.region+self.band+'.fits'
