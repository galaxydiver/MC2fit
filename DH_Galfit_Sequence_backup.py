...
def multicore_crawling(DirInfo=None, dir_group=None, region='s',
    ...
    global sub_multicore
    def sub_multicore(i):
        global sub_crawling
        def sub_crawling(i):
            coord=FG_using.coord_array_todo[i]
            dir_work=FG_using.dir_work_list_todo[i]
        ...
        if(show_progress>0):
            if(i%show_progress==0): print(i, "/", len(FG_using.coord_array_todo), "| Time : ", time.time()-start)
    ...
    if(Ncore>1):
        if(timeout==None): ## Normal multi-core
            ...
            dummy=pool.map(sub_multicore, np.arange(len(FG.coord_array_todo)))
            pool.close()
            pool.join()
            ...
            band='r'
            pool = Pool(processes=int(Ncore))
            dummy=pool.map(sub_multicore, np.arange(len(FGr.coord_array_todo)))
            pool.close()
            pool.join()
    ...
    #             future = pool.map(sub_crawling, np.arange(len(coord_array_todo)), timeout=timeout)
            ...
            FG_using=FG
            band='g'
            future = pool.map(sub_multicore, np.arange(len(FG.coord_array_todo)), timeout=timeout)
            pool.close()
            pool.join()
            ...
            band='r'
            pool = ProcessPool(int(Ncore))
            future = pool.map(sub_multicore, np.arange(len(FGr.coord_array_todo)), timeout=timeout)
            pool.close()
            pool.join()
    else:
        for i in range (len(FG.coord_array_todo)):
            ...
            band='g'
            sub_multicore(i)
        for i in range (len(FGr.coord_array_todo)):
            FG_using=FGr
            band='r'
            sub_multicore(i)
    ...
    def sub_extract_fits(i):
        ...
        if(len(existlist1)!=0):
            ...
            for j, fn in enumerate(existlist1):
                newfn=newfnlist[j]
                fits.writeto(newfn, fits.getdata(fn, 2), fits.getheader(fn, 2),
...
class ResPack:
    def __init__(self, Runlist, proj_folder, DirInfo, comp_autoswap=True, swapmode='N',
        ...
        self.comp_4th_autoswap=comp_4th_autoswap
    ...
    def _getdata(self, proj_folder, runlist, dir_work_list,
        ...
        def sub_getdata(i):
            Res=dhgalfit.ResultGalfit(proj_folder=proj_folder, runlist=runlist,
                                     ...
                                     comp_4th_autoswap=self.comp_4th_autoswap,
                                     )
    ...
    def preset_generate_onlyval(self, submodel=-1, silent=False):
        ...
        self._get_add_params_array(submodel)

    def _get_add_params_array(self, submodel=-1):
        if(submodel==-1): ##best of best
            self.is_sersic, self.add_params1_array, self.add_params2_array, self.add_params3_array\
            = dhgalfit.convert_add_params_array(self.bob_onlyval)
            ...
            = dhgalfit.convert_add_params_array(self.bobwarn_onlyval)
            ...
            = dhgalfit.convert_add_params_array(self.thismodel_onlyval)
            ...
            = dhgalfit.convert_add_params_array(self.thismodelwarn_onlyval)
