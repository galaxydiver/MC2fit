...
def pix2mu(array, plate_scale=0.262, m_zero=22.5):
    # array : pixel values --> flux / pixel2
    f=array/plate_scale**2  # flux/arcsec2
    return m_zero-2.5*np.log10(f) # mag / asec2
...
class AutoGalfit:
    ...
    def _set_path_name(self):
        self.dir_work_galfit=self.dir_work+self.proj_folder
        os.makedirs(self.dir_work_galfit, exist_ok=True)

        self.fn_gal_conf = self.dir_work_galfit + 'param_' + self.prefix + self.outputname+ self.suffix + ".dat" # Configuration file
        self.fn_constraint=self.dir_work_galfit + 'constraints.dat'  ## We can use a single constraint file
        ...
        if(self.fna_inputmask=='none'): self.fn_inputmask = 'none'  # Masking file
            ...
            except: raise Exception(">> ♣♣♣ Warning! ♣♣♣ Masking file does not exist! -"+str(self.fn_inputmask))
        self.fn_output_imgblock=self.dir_work_galfit+'result_'+ self.prefix + self.outputname + self.suffix + ".fits"
...
def drawing_galfit(fn='./TestRun/result_test.fits', gs=None, data_index=0, silent=False,              #Basic
    ...
    for i in range (3):
        ...
        if(vmin!=None):
            a=ax0.imshow(imagedata, cmap='bone', origin='lower', vmin=vmin, vmax=vmax)
            if(show_colorbar): plt.colorbar(a)

        elif(scale_percentile==None):
            a=ax0.imshow(imagedata, cmap='bone', origin='lower')
            if(show_colorbar): plt.colorbar(a)
            vmin, vmax=a.get_clim()

        else:
            newscale=[np.percentile(imagedata, 50-scale_percentile), np.percentile(imagedata, 50+scale_percentile)]
            a=ax0.imshow(imagedata, cmap='bone', origin='lower', vmin=newscale[0], vmax=newscale[1])
            if(show_colorbar): plt.colorbar(a)
            vmin, vmax=a.get_clim()
...
#                 default  [center|center|20 | 20 | 4 |1 |0 ]
            ...
            if(thisrunlist['use_lim']==1):
                run.add_constraints(id_comp=2, poslim=thisrunlist['lim_pos1'],
            ...
            if(len(thisrunlist['compname'])>1):
                ...
                if(thisrunlist['use_lim']==1):
                    run.add_constraints(id_comp=3, poslim=thisrunlist['lim_pos2'],
...
##======================== Reduced chi2 ========================
def cal_update_chi2(fnlist, fn_sigma, fn_masking=None, radius=-1, repair_fits=True, image_size=200,
                    header_item='Chi_50', header_item_aic='AIC_50', overwrite=False, silent=True):
    """
    ...
    ** See cal_chi2()
    ...
    for fn in fnlist:
        ...
        chi2, Npixel=cal_chi2(dat, sig, mask, radius)
        if(overwrite==False):
        ...
        try: aic=chi2+2*p+(2*p*(p+1))/(Npixel-p-1)
        except: aic=999999
...
def cal_chi2(dat, sig, mask=None, radius=-1):
    ...
    return np.sum((mdat[innerpart]**2/sig[innerpart]**2)), len(innerpart[0])
        ...
        try:
            ...
            for j, name in enumerate(itemlist):
                try:
                    ...
                    raise Exception(">> ♣♣♣ Warning! ♣♣♣ Header has problems!"+str(fnlist[i]))
                    #errorcount+=1
                    continue
        except:
...
class ResGalData:
    ...
    def read_data(self, itemlist):
        fnlist=self.fnlist
        chi2itemlist=['CHI2NU', 'AIC_F', 'RChi_50', 'AIC_50']
...
class ResultGalfit:
    def __init__(self, proj_folder='galfit/',
                 ...
                 fn_load='', fna_load='saved_resgal.dat', group_id=None,  ## 1) Load data
                 tolerance=1e-3, silent=False, auto_input=True, ignore_no_files=False,
                 comp_crit='RChi_50', input_only=False,
                 extended_itemlist=['CHI2NU', 'AIC_F', 'RChi_50', 'AIC_50', 'war_tot', 'group_ID'],
                 comp_autoswap=False, swapmode='MU'
                ):
        """
        ...
        self.galfit_itemlist_2nd=['2_XC', '2_YC', '2_MU_E', '2_RE', '2_N', '2_AR', '2_PA', '2_MAG']
        self.galfit_itemlist_3rd=['3_XC', '3_YC', '3_MU_E', '3_RE', '3_N', '3_AR', '3_PA', '3_MAG']
        ...
        if(fna_load==None): fn_load=None
            ...
            fn_load=self.dir_work_galfit+fna_load
    ...
    def input_load(self, fn_load, group_id=None):
        ...
        ** See input_runlist as well
        ...
        if(self.silent==False):
            try: print("\n>> Replace dir_work_galfit : ", self.dir_work_galfit, "to", header3)
            except: print("\n>> New dir_work_galfit : ", header3)
        self.dir_work_galfit = header3
    ...
    def find_best(self, remove_warn=True, comp_crit='RChi_50'):
    ...
    def show_data(self, only_val=False, hide_nan=True, hide_PA=True, hide_fail_read=True):
        ...
        newitemlist_chi2=np.append(newitemlist, ['CHI2NU', 'AIC_F', 'RChi_50', 'AIC_50'])
    ...
    def make_swaplist(self, mode='RE'):
        if(mode=='RE'):
            ...
            for i in range (len(self.Data.val)):
                ...
                s2=Sersic1D(amplitude=mu2pix(self.Data.val[i]['2_MU_E']), r_eff=self.Data.val[i]['2_RE'], n=self.Data.val[i]['2_N'])
                s3=Sersic1D(amplitude=mu2pix(self.Data.val[i]['3_MU_E']), r_eff=self.Data.val[i]['3_RE'], n=self.Data.val[i]['3_N'])
...
def convert_base_params_array(usingdat):
    ...
    ## For 3rd comp, if the data has other information except Mu_E --> it might be PSF
    is_psf=np.isfinite(base_params2_array[:,0]) &  np.isnan(base_params2_array[:,2])
    is_sersic=np.isfinite(base_params2_array[:,0]) &  np.isfinite(base_params2_array[:,2])
    base_params2_array[is_psf,2]=usingdat[is_psf]['3_MAG'] # Mu_e = mag
    return is_sersic, base_params1_array, base_params2_array
...
class PostProcessing:
    ...
    chi_radius : int = 100
    image_size : int = 200
    repair_fits : bool = True
    header_item : str = 'Chi_50'
    header_item_aic : str = 'AIC_50'
    ...
    def __post_init__(self):
        ...
        if((self.fn_sigma=='') & (self.fna_sigma!=None)):
            self.fn_sigma=self.dir_work+self.fna_sigma
        if((self.fn_masking=='') & (self.fna_masking!=None)):
            self.fn_masking=self.dir_work+self.fna_masking
    ...
    def post_chi2(self):
        ...
        fnlist=dharray.array_attach_string(self.Runlist.namelist, 'result_', add_at_head=True)
        fnlist=dharray.array_attach_string(fnlist, self.dir_work+self.proj_folder, add_at_head=True)
        fnlist=dharray.array_attach_string(fnlist, '.fits')
        self.fnlist=np.array(fnlist)
        cal_update_chi2(self.fnlist, self.fn_sigma, self.fn_masking, -1,
                        self.repair_fits, self.image_size,
                        'Chi_F', 'AIC_F', self.overwrite, silent=self.silent)
        cal_update_chi2(self.fnlist, self.fn_sigma, self.fn_masking, self.chi_radius,
                        self.repair_fits, self.image_size,
                        self.header_item, self.header_item_aic, self.overwrite, silent=self.silent)
    ...
    def centermag_sersic(self):
        ...
        tmag2, sersiclist2=add_totalmag(self.ResThis.Data.val, comp=2,
                                        sersic_res=self.sersic_res, plate_scale=self.plate_scale)
    ...
    def centermag_galfit(self):
        if(self.silent==False): print(">> Galfit Centermag")
        fnlist_center=dharray.array_attach_string(self.Runlist.namelist, 'result_cimsi_', add_at_head=True)
        fnlist_center=dharray.array_attach_string(fnlist_center, self.dir_work+self.proj_folder, add_at_head=True)
        fnlist_center=dharray.array_attach_string(fnlist_center, '.fits')
        self.fnlist_center=np.array(fnlist_center)

        is_sersic, base_params1_array, base_params2_array=convert_base_params_array(self.ResThis.Data.val)
        sersiclist2=np.arange(len(base_params1_array))
        sersiclist3=np.arange(len(base_params1_array))[is_sersic]
        self.Runlist.run_galfit_with_runlist(dir_work=self.dir_work, proj_folder=self.proj_folder,
                        fna_inputimage=self.fna_image, fna_inputmask=self.fna_masking, ## Masking -> same region
                        fna_psf=self.fna_psf, fna_sigma=self.fna_sigma,
                        ...
                        overwrite=self.overwrite, prefix='cimsi_', suffix='', silent=self.silent,
                        print_galfit=self.print_galfit)

        # print("Sersic List", is_sersic)
        # print("Sersic List2", sersiclist2)
        # print("Sersic List3", sersiclist3)
        ## It will use fna_ not fn_!
        for i, fn in enumerate (self.fnlist): ## Original FN_list
            if(self.ResThis.Data.is_file_exist[i]==False): continue
            try:
                head=fits.getheader(self.fnlist_center[i], ext=2) ## New Fn_list_center
            except:
                if(self.silent==False): print(">> File read error! ", self.fnlist_center[i])
                continue

            tmag2=head['2_MAG'] ## Get value from fn_list_center and input to the original one
...
# def postprocessing(proj_folder='galfit/', Runlist=None, dir_work='',
#                    fn_sigma='', fn_masking='',
...
#                    centermag='sersic', sersic_res=500, plate_scale=0.262,
...
#     fnlist=dharray.array_attach_string(fnlist, dir_work+proj_folder, add_at_head=True)
#     fnlist=dharray.array_attach_string(fnlist, '.fits')
...
#     ##centermag
#     if(centermag!=None):
#         ResThis=ResultGalfit(proj_folder=proj_folder, runlist=Runlist.runlist,
#                                 dir_work=dir_work, group_id=None, auto_input=True,
#                                 fna_load=None,
#                                 silent=True, ignore_no_files=True)
#         ###================ Mode 1 : Sersic calc. =================
#         if(centermag=='sersic'):
#             tmag2, sersiclist2=add_totalmag(ResThis.Data.val, comp=2,
#                                             sersic_res=sersic_res, plate_scale=plate_scale)
#             tmag3, sersiclist3=add_totalmag(ResThis.Data.val, comp=3,
#                                             sersic_res=sersic_res, plate_scale=plate_scale)
#             ## INPUT
...
#             fnlist_center=dharray.array_attach_string(fnlist_center, dir_work+proj_folder, add_at_head=True)
#             fnlist_center=dharray.array_attach_string(fnlist_center, '.fits')
#             fnlist_center=np.array(fnlist_center)
#
#             is_sersic, base_params1_array, base_params2_array=convert_base_params_array(ResThis.Data.val)
#             print("Sersic List", is_sersic)
#             sersiclist2=np.arange(len(base_params1_array))
#             sersiclist3=np.arange(len(base_params1_array))[is_sersic]
...
#                             is_allow_vary1=False, is_allow_vary2=False,
#                             is_2nd_mag_offset=None,
#                             overwrite=overwrite, prefix='cimsi_', suffix='', silent=silent,
...
class ResultGalfitBest(ResultGalfit):
    def __init__(self, MomRes, silent=True,
                 include_warn=False, get_single=False, comp_crit='RChi_50'):
        self.silent=silent
        self.MomRes=MomRes
        self.galfit_itemlist=MomRes.galfit_itemlist
        self.dir_work=MomRes.dir_work
        self.dir_work_galfit=MomRes.dir_work_galfit
...
def galfit_swap_band(runlist, ResBest, silent=False, print_galfit=False):
    ...
    for i in range (len(using_best_submodels)):
        ...
        swap=AutoGalfit(fn_galfit=fn_galfit, dir_work=dir_work, proj_folder=proj_folder,
        ...
        cal_update_chi2([fitsname], dir_work+'sigma_r.fits', radius=100,
...
class ResultGalfitSwap(ResultGalfit):
    def __init__(self, OriResBest=None,  ## For input_runlist mode
        ...
        self.extended_itemlist=['CHI2NU', 'AIC_F', 'RChi_50', 'AIC_50', 'war_tot', 'group_ID']
        self.ignore_no_files=ignore_no_files
        self.tolerance=tolerance  # Chi2 tolerance for "Good results" (|Min chi2 - chi2 |/(Min chi2) < tolerance)
        self.silent=silent
    ...
    def input_copy(self):
        ...
        for i in range (len(using_best_submodels)):
            ...
            ImsiRes=ResultGalfit(project_folder=self.OriResBest.Data, runlist=None, fna_load=None, dir_work=self.OriResBest.dir_work, group_id=None,
