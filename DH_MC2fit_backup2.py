...
class PyGalfit():
    ...
    def add_constraints(self, id_comp, lim_params=np.full((7,2), np.nan), re_cut=None, n_ratio=None):
        ...
        if(hasattr(re_cut, "__len__")):
            if(len(re_cut)==2):
                lim_params[3,0]=np.nanmax([lim_params[3,0], re_cut[0]])
                lim_params[3,1]=np.nanmin([lim_params[3,1], re_cut[1]])
            else:
                lim_params[3,0]=np.nanmax([lim_params[3,0], re_cut])
...
class MC2fitRun_Mulcore():
    ...
    def __init__(self,
        ...
        self.base_params_array_set=base_params_array_set
...
class Result():
    ...
    def _post_init(self):
        ...
        self.plate_scale=dharray.value_repeat_array(self.plate_scale, 2)
        self.image_size=dharray.value_repeat_array(self.image_size, 2)
        self.img_center=self.image_size/2
        self._input()
        self._dataswap()
    ...
    def _dataswap(self):
        ...
        if((self.comp_4th_ss_autoswap) & (np.isin(['4_XC'], self.Data.val.dtype.names)[0])):
            ...
            dist2=(self.Data.val['2_XC']-self.img_center[0])**2 + (self.Data.val['2_YC']-self.img_center[1])**2
            dist3=(self.Data.val['3_XC']-self.img_center[0])**2 + (self.Data.val['3_YC']-self.img_center[1])**2
